...
%% SUBSET DATASET TO TRAIN {G3, G4} and TEST {G3/4}
...
for i = 0:length(classVector)-1
    ...
    if (classVector(i+1) == 1 || classVector(i+1) == 3)  % Add G3, G4 to TRAIN
        TRAIN.add(D.instance(i));
        trainValues = [trainValues classVector(i+1)];
    elseif (classVector(i) == 2)                     % Add G34 to TEST
        TEST.add(D.instance(i));
    end
    
end

trainValues(trainValues ==1) = 0;  % Map old values to new indices
trainValues(trainValues ==3) = 1;

% Create new attribute
newAttribute = wekaCreateAttribute('nlabel', 'nominal', {'G3', 'G4'});
...
% And write to file
output_filename = 'G3-G4_consolidated.arff';
